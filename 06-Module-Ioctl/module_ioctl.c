...
/* Add the declaration of the unlocked ioctl function */
static long example_unlocked_ioctl(struct file *, unsigned int, unsigned long);

/* Add the delcaration of the compat ioctl function */
...
/*
 * Definition of the unlocked ioctl function
 * We don't use the structure defined in module_ioctl.h here but we could , this
 * is just an example.
 */
static long example_unlocked_ioctl(struct file *filep, unsigned int cmd,
				 unsigned long arg)
{
	switch (cmd) {
	case EXAMPLE_IOCTL:
		pr_info("Called EXAMPLE_IOCTL");
	break;
	case EXAMPLE_IOCTL_W:
		pr_info("Called EXAMPLE_IOCTL_W");
	break;
	case EXAMPLE_IOCTL_R:
		pr_info("Called EXAMPLE_IOCTL_R");
	break;
	case EXAMPLE_IOCTL_WR:
		pr_info("Called EXAMPLE_IOCTL_WR");
	break;
	default:
	break;
	}
...
/* Definition of the compat ioctl function
 * The compat_ioctl is here to allow a 32bit process to call ioctl() on a 64bit
 * kernel
 */
static long example_compat_ioctl(struct file *filep, unsigned int cmd,
				 unsigned long arg)
{
